...
class Produto:
    def __init__(self, id_produto=None, nome_produto=None, tipo_produto=None, qtd_produto=None, imagem_produto=None):
    ...
    def cadastrarProduto(dsn, id_produto, listaProdutos):
        ...
        # SETANDO O TIPO DO PRODUTO - OK
        tipo_produto = float(input("DIGITE O TIPO DO PRODUTO: "))
        tipo_produto = Funcoes.validarPreenchimento("DIGITE O TIPO DO PRODUTO: ", tipo_produto)
        novo_produto.tipo_produto = tipo_produto
        ...
        novo_produto = Produto(id_produto = id_produto, nome_produto = nome_produto, tipo_produto = tipo_produto, qtd_produto = qtd_produto, imagem_produto = imagem_produto)
        ...
        cursor.execute("INSERT INTO produto (id_produto, nome_produto, tipo_produto, qtd_produto, imagem_produto) VALUES (:1, :2, :3, :4, :5)", (novo_produto.id_produto, novo_produto.nome_produto, novo_produto.tipo_produto, novo_produto.qtd_produto, novo_produto.imagem_produto))
    ...
    def editarProduto(self, dsn, listaProdutos):
        ...
        while (perfilProduto == 1):
            ...
            if (opcao == 1):
                ...
                # (ADMIN) EDITAR O TIPO DO PRODUTO - OK
                opcao = int(input(Funcoes.confirmarAcao("EDITAR TIPO")))
                opcao = int(Funcoes.validarOpcao(opcao, 1, 2, Funcoes.confirmarAcao("EDITAR TIPO")))

                if (opcao == 1):
                    tipo_novo = float(input("DIGITE O NOVO TIPO: "))
                    tipo_novo = Funcoes.validarPreenchimento("DIGITE O NOVO TIPO: ", tipo_novo)
                    ...
                    # EDITANDO NO BANCO DE DADOS E NO CONSOLE
                    cursor.execute("UPDATE produto SET tipo_produto = :tipo_novo WHERE id_produto = :id_produto", {"tipo_novo": tipo_novo, "id_produto": produto_buscado.id_produto})
                    cursor.connection.commit()
                    produto_buscado.tipo_produto = tipo_novo
                    print("TIPO DO PRODUTO EDITADO COM SUCESSO!")
                    input("TECLE ENTER PARA VOLTAR AO MENU.")
    ...
    def perfilProduto(produto_buscado):
        ...
        retornoPerfil += f"03. TIPO: {produto_buscado.tipo_produto}\n"
